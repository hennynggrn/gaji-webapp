...
class M_gaji extends CI_Model{
	...
	public function get_pinjaman($id_pegawai = TRUE, $kode, $status = TRUE)
	{
		$month = date('m');
		$year = date('Y');
		if ($id_pegawai != NULL) {
			switch ($kode) {
				case 'KOP':
					...
					if ($status != NULL) {
						...
						$this->db->group_by('p.id_pegawai');
						$data = $this->db->get_where('pegawai p', array('pjm.id_pegawai' => $id_pegawai));
					} else {
						$this->db->join('pinjaman pjm', 'p.id_pegawai = pjm.id_pegawai AND pjm.kode_pinjaman = "'.$kode.'"', 'LEFT OUTER');
						$this->db->join('angsuran a', 'a.id_pinjaman = pjm.id_pinjaman AND (month(a.tanggal_kembali) = "'.$month.'" AND year(a.tanggal_kembali) = "'.$year.'")', 'LEFT OUTER');
						$this->db->group_by('p.id_pegawai');
						$data = $this->db->get_where('pegawai p', array('a.payOff_byGaji' => 1));
					}
					return $data;
					break;
